namespace Auto3D.Services.Data
{
    ...
    public class CarsService : ICarsService
    {
        ...
        public async Task<int> EditAsync(CarEditViewModel carEditModel)
        {
        ...
        public IEnumerable<CarViewModel> GetProductsFromSearch(string searchString, int brandId)
        {
            var escapedSearchTokens = searchString.Split(new[] { ' ', ',', '.', ':', '=', ';' }, StringSplitOptions.RemoveEmptyEntries);

            var carBrand = this.carBrandsService.GetBrand(brandId);

            var cars = this.carsRepository
           .AllAsNoTracking()
           .Where(p => p.BrandId == brandId)
           .ToArray()
           .Where(c => escapedSearchTokens.All(t => c.Model.ToLower().Contains(t.ToLower())) ||
                        escapedSearchTokens.All(t => c.Year.ToLower().Contains(t.ToLower())))
           .Select(x => new CarViewModel
           {
               Id = x.Id,
               Model = x.Model,
